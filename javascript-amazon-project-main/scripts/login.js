...
document.querySelector('.js-user-login-button').addEventListener('click', async (event)=>{
    ...
    let promise = await fetch(`/api/userlogin?userName=${userName}&password=${password}`)
    ...
    document.querySelector('.js-seller-login-button').addEventListener('click', async (event)=>{
        ...
        let promise = await fetch(`/api/sellerLogin?sellerName=${sellerName}&password=${password}`)
          .then(response => response.json())
          .then(data => {
              if (data.message === 'success') {
                  console.log(data);
